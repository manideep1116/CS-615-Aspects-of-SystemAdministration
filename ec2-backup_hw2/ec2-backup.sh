...
instance=`aws ec2 run-instances --image-id  ami-569ed93c --count 1 --instance-type t1.micro --key-name ec2-backup --output json --placement AvailabilityZone=$avail_zone`
...
disp=`echo $instance |  awk '{print $48}' | sed 's/^"\(.*\)".*/\1/'`
...
ssh  -o StrictHostkeyChecking=no root@$pubDNS  'echo $(uname -a); echo $(whoami); echo $(/sbin/disklabel xbd2)' 
